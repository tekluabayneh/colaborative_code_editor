...
const sendInvite = async (req:Request, res:Response) => { 
	...
	const {email, role } = req.body
     ...
     if(await validator.isUserRoleOwnerOrUser(email) || await InviteToken.findOne({email})){ 
	res.status(400).json({message:"user already exist or it's in invite proccess"})
		return 
	}
	...
	const storetoken = await InviteToken.insertOne({token:token, email:email, role:role, InvitedBy:inviterId, expireAt:expiresAt})
...
const acceptInvite = async (req: Request, res: Response) => {
  	...
  	const requiredFields = ["userName", "email", "password", "token", "role", "invitedBy"] as const 
  ...
  try {
    ...
    const StoreUser = await Users.insertOne({ userName, email, password: hashedPassword, role, invitedBy , createdAt: new Date()});
